    ...
    <div class="row g-2 mb-3">
        ...
        <div class="col-md-9">
            <?php if (info_pemilu() == 0) : ?>
                <a href="<?= base_url(); ?>public/ppdb" <?= (url() == '' ? 'target="_blank"' : ''); ?>>
    ...
    <div class="row g-2 mb-3">
        <div class="col-md-8">
            <div class="card" style="max-width: 100%;">
                <img src="<?= base_url(); ?>berkas/news/<?= $data['img']; ?>" class="card-img-top" alt="<?= $data['judul']; ?>">
                <div class="card-body">
                    <h5 class="card-title"><?= $data['judul']; ?></h5>
                    <span style="color: #054552;"><i class="fa-regular fa-clock"></i></span> <?= date('d/m/Y H:i:s', $data['tgl']); ?> <a href="<?= base_url(); ?>public/news/label/<?= $data['label']; ?>" style="text-decoration: none;color:#054552"><i class="fa-solid fa-tag"></i> <?= $data['label']; ?></a> <a style="text-decoration: none;color:#054552" href="<?= base_url(); ?>public/news/username/<?= $data['username']; ?>"><i class="fa-solid fa-user"></i> <?= $data['penulis']; ?></a>
        ...
        <div class="col-md-4">
            ...
            <?php foreach (get_news('Trending') as $k => $i) : ?>
                <?php if ($k < 11) : ?>
                    <a <?= (url() == '' ? 'target="_blank"' : ''); ?> href="<?= base_url(); ?>public/news/single/<?= $i['slug']; ?>" class="card mb-2" style="max-width: 100%;text-decoration:none;color:#666666">
                        <div class="row g-0">
    ...
    <div class="row g-2">
        ...
        <div class="col-md-6">
            ...
            <?php foreach (labels_news() as $i) : ?>
                <div class="mb-2">
                    <div class="d-grid mb-1">
                        <a class="btn_secondary" <?= (url() == '' ? 'target="_blank"' : ''); ?> style="border-color: #666666;border-radius:3px;" href="<?= base_url(); ?>public/news/label/<?= $i['label']; ?>"><?= $i['label']; ?></a>
                    </div>
                    <?php foreach (get_news($i['label']) as $n) : ?>

                        <div class="mb-1">
                            <a <?= (url() == '' ? 'target="_blank"' : ''); ?> href="<?= base_url(); ?>public/news/single/<?= $n['slug']; ?>" class="btn_main_inactive d-flex gap-2" style="border-bottom: 1px solid #666666; border-radius:0px;">
                                <img style=" max-width: 70px;" src="<?= base_url(); ?>berkas/news/<?= $n['img']; ?>" class="img-fluid rounded" alt="<?= $n['judul']; ?>">
                                <div><?= $n['judul']; ?></div>
                                <div><?= date('d/m/Y H:i:s', $n['tgl']); ?></div>
                            </a>
                        </div>
                    <?php endforeach; ?>

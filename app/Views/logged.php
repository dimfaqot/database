            ...
            <div class="offcanvas-body">
                <?php foreach (menus() as $i) : ?>
                    <div class="mb-1 d-grid">
                        <a href="<?= base_url(); ?><?= $i['controller']; ?><?= ($i['controller'] == 'informasi' ? '/Recruitment/Member/L' : ($i['controller'] == 'menu' ? '/Root' : ($i['controller'] == 'user' ? '/Recruitment' : ($i['controller'] == 'options' ? '/Role' : ($i['controller'] == 'rebana' || $i['controller'] == 'rental' ? '/' . date('Y') . '/' . date('m') : ($i['controller'] == 'karyawan' || $i['controller'] == 'recruitment' ? '/Existing/1/SMP/updated_at/DESC/' . ($i["controller"] == "karyawan" ? "Aktif" : "Register") . '/All' : ($i['controller'] == 'sk' ? '/All/' . date('Y') . '/updated_at/DESC' : ($i['controller'] == 'santri' || $i['controller'] == 'ppdb' ? '/' . ($i['controller'] == 'santri' ? 'All' : tahun_santri('ppdb')) . '/Existing/1/SMP/updated_at/DESC/' . ($i["controller"] == "santri" ? "Aktif" : "Register") . '/All' : ($i['controller'] == 'pilangsari' ? '/Karyawan/1/updated_at/DESC/SSJ'  : ($i['controller'] == 'piagam' ? '/Dinas/' . date('Y') : ($i['controller'] == 'mapel' ? '/MUL' : ($i['controller'] == 'nilai' ? '/' . date('Y') . '/MUL' : ($i['controller'] == 'calon' || $i['controller'] == 'hasil' ? '/' . date('Y') : ($i['controller'] == 'pemilih' ? '/' . date('Y') . '/Belum/1/Karyawan/Putra/updated_at/DESC' : ($i['controller'] == 'artikel' ? '/label/Headlines/1/updated_at/DESC' : ''))))))))))))))); ?>" style="font-size: small;" class="px-3 py-1 <?= (url() == $i['controller'] ? 'btn_main' : 'btn_main_inactive'); ?>"><i class="<?= $i['icon']; ?>"></i> <?= $i['menu']; ?></a>
                    </div>
                <?php endforeach; ?>
